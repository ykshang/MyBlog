...
- LZ77 压缩阶段：查找重复的字符串，使用 `<距离,长度>` 替换。
- 哈夫曼编码阶段：生成哈夫曼编码表，根据哈夫曼编码表对字符进行编码。
- 封装输出：添加文件头（含元信息）、添加 CRC 校验码、生成 `.gz` 后缀的压缩文件。
...
zstd 是 Facebook 开发的一种新的压缩算法，它在压缩效率和压缩速度之间取得了平衡。与 Gzip 和 Brotli 相比，zstd 提供更高的压缩率，但压缩速度也相对较慢。
