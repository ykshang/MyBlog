...
联想一下：我们在遍历数组时，一般不会在循环里对数组进行`push`、`splice`、`pop` 等操作，而是使用一个新的数组去存放处理好的元素，就是怕在遍历中修改数组会打乱遍历结果。

同样的道理，Vue 中同级比较只存在三种情况：**新增**（插入、追加子节点）、**删除**（删除子节点）、**移动**（先插入、后删除）。如果采用传统的循环遍历去对比新旧节点树，并在循环中执行节点的插入、删除、移动等操作，会导致真实节点的结构、顺序发生不可预期的变化，从而影响了下一次循环的比较结果。

而我们希望的是： diff 执行完以后，DOM 结构与 newVnode 的结构是一致的。双端交叉对比就很好的满足了这个需求
