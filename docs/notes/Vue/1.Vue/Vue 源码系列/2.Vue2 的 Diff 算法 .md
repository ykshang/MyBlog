...
## 三、为什么 Vue2 会使用双端比较算法？

联想一下，我们在遍历数组时，一般不会直接对数组进行增删和修改长度。而是使用一个新的数组去存放处理好的元素。就是怕在遍历中修改数组会影响到数组的遍历结果。

vue 的 diff 算法是比较的是 oldVnode 和 newVnode，最终是通过操作 oldVnode 里节点指向的 dom 元素完成比较的更新。而子元素的变化只有三种情况：

- 新增：插入子节点、追加子节点
- 删除：删除子节点
- 移动：先插入、后删除

我们希望的是 diff 执行完以后，DOM 结构与 newVnode 的结构是一致的。如果我们在循环遍历中，直接直接几种操作都会直接改变 DOM 树，会导致真实节点的结构、顺序发生变化，产生了不可预期的变化，从而影响了下一步的比较的结果。
