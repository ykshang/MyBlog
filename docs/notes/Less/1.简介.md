---
title: 简介
createTime: 2025/04/13 18:24:10
permalink: /Less/ok2j3222/
---
...
### **10、转义（Escape）** <badge text="已弃用" type="danger" />

使用 `~` 转义字符，避免特殊字符解析错误。在 LESS 3.5 及以上版本中，`~`语法已被弃用，推荐使用 `e()` 函数或直接引号包裹：
...
.element {
  width: ~"calc(100% - 20px)"; // LESS 不会计算表达式
  font-family: ~"Arial, 'Helvetica Neue'"; // 保留字符串中的引号和逗号
}
```
