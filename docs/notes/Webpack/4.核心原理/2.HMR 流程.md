...
sequenceDiagram
    ...
    loop 监听文件变化
        ...
        浏览器->>DevServer: 请求 manifest 数据 <br/> JSONP / Ajax
...
浏览器（即`runtime`）收到 `ok` 消息后，根据 `hash` 消息中携带的 `hash` 值，向 devServer 发起 JSONP 请求 Manifest 来获取更新清单。
...
Webpack 5 中支持多种请求方式，包括：JSONP、EventSource、Fetch API。
