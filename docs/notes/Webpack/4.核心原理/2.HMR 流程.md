...
Webpack 在打包时自动注入 `webpack-dev-server/client` 和 `webpack/hot/dev-server` 代码，用于管理 HMR 通信。

### 建立 WebSocket 连接

浏览器与开发服务器建立 WebSocket 连接，用于实时通信。

## 文件变更检测

### 监听文件变化

开发服务器（如 `webpack-dev-server`）使用 `chokidar` 监听文件系统的变化。
...
1. `hash` 消息：携带最新编译的 `hash` 值。
2. `ok` 消息：表示编译完成，可以开始更新。
...
sequenceDiagram
    ...
    loop 监听文件变化
        ...
        DevServer->>Webpack: 触发重新编译
        Webpack->>Webpack: 增量编译
        Webpack->>Webpack: 生成补丁文件
        Note right of Webpack: 补丁文件包含<br/>1. [hash].hot-update.json<br/>2. [moudle].hot-update.js
        Webpack->>DevServer: compilation.hooks.done <br/> 编译完成
        DevServer->>浏览器: 发送 hash 消息
        浏览器->>DevServer: 请求 manifest.json
        DevServer->>Webpack: 读取 hot-update.json
        Webpack->>DevServer: 返回增量 manifest.json
        DevServer->>浏览器: 响应 manifest.json 请求
        浏览器 ->> DevServer: 请求补丁文件
