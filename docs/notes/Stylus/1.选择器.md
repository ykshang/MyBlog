...
1. 用==缩进==代替 `{` 和 `}`。
2. 分隔属性和值的 `:` ==冒号可以省略==。
3. ==建议保留冒号==，因为它可以明确区分 ==属性: 值== 和 ==群组选择器==。
4. 用==换行==代表群组选择器，某些场景下，建议通过保留逗号来提高可读性。
5. 行末尾的 `;` ==分号可以省略==。
...
**注意**：有些选择器看起来像属性。例如，下面的`foo bar baz`可能是属性或选择器：
...
该内置函数还可以接受一个可选的字符串参数，这种情况下它会返回编译后的选择器。需要注意的是，如果参数中不包含任何`&`符号，它不会在当前作用域的选择器前添加前缀。
