---
title: 10. interface 接口
createTime: 2025/05/02 12:40:14
permalink: /TypeScript/o9dhmyzx/
copyright:
  creation: reprint
  license: CC-BY-4.0
...
上面示例中，接口`Circle`和`Rectangle`组成一个联合类型`Circle | Rectangle`。因此，这个联合类型的同名属性`area`，也是一个联合类型。本例中的`declare`命令表示变量`s`的具体定义，由其他脚本文件给出，详见[《declare 命令》](/MyBlog/TypeScript/hj676k7f/)一章。
...
- `interface`只能表示对象类型（包括数组、函数等）。

### 2、添加属性（`interface`继承、`type &`）

- `interface`可以继承其他类型，`type`不支持继承：继承的主要作用是添加属性

- `type`定义的对象类型如果想要添加属性，只能使用`&`运算符，重新定义一个类型。

例如：

以下示例中，类型`Bear`在`Animal`的基础上添加了一个属性`honey`。`&`运算符，表示同时具备两个类型的特征，因此可以起到两个对象类型合并的作用。
...
继承属性时，`type`和`interface`是互相继承的。

Interface 可以使用`extends`继承 Type。
...
- Interface 支持重复定义一个同名类型，同名的 Interface 会自动合并，
- 而 type 多次定义一个同名类型则会报错。

以下示例中，`type`两次定义了类型`A`，导致两行都会报错：
...
`interface`不能包含属性映射（mapping），`type`可以，详见[《映射》](/MyBlog/TypeScript/yc990ssc/)一章。
