...
对象的类型检查是严格的。一旦声明了类型，在给对象赋值时，就不能缺少指定的属性，也不能有多余的属性，同时值的类型需要和类型约束严格匹配。
...
user.age = "20"; // Error: 不能将类型“string”分配给类型“number”。ts(2322)
user.sex = "male"; //Error: 类型“User”上不存在属性“sex”。ts(2339)
```
...
user.name = "Bob"; // Error: 无法为“name”赋值，因为它是只读属性。ts(2540)
```
...
bob = alice; // 报错：不能将类型“{ readonly name: string; readonly age: number; readonly isAdult: boolean; }”分配给类型“{ name: string; age: number; isAdult: boolean; }”。ts(2322)
```

测试发现：`bob`的值可以赋给`alice`，但是`alice`的值不能赋给`bob`，所以`bob`是`alice`的子类型。
