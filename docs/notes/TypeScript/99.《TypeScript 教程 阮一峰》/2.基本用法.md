...
permalink: /notes/TypeScript/mqt0evkb/
copyright:
  creation: reprint
  license: CC-BY-4.0
...
上面示例中，函数`toString()` 的参数`num` 的类型是`number`。参数列表的圆括号后面，声明了返回值的类型是`string`。更详细的介绍，参见[《函数》](./8.function%20函数.md) 一章。
...
tsc 命令的更多参数，详见[《tsc 编译器》](./30.tsc%20命令行编译器.md)一章。
...
`tsconfig.json`的详细介绍，参见[《tsconfig.json 配置文件》](./28.tsconfig.json%20配置文件.md)一章。
