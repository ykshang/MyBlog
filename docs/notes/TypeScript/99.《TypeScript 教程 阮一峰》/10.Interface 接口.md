...
permalink: /notes/TypeScript/o9dhmyzx/
copyright:
  creation: reprint
  license: CC-BY-4.0
...
上面示例中，接口`Circle`和`Rectangle`组成一个联合类型`Circle | Rectangle`。因此，这个联合类型的同名属性`area`，也是一个联合类型。本例中的`declare`命令表示变量`s`的具体定义，由其他脚本文件给出，详见[《declare 命令》](./19.declare%20关键字.md)一章。
...
`interface`不能包含属性映射（mapping），`type`可以，详见[《映射》](./22.Mapping%20类型映射.md)一章。
