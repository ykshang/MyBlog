---
title: 14. Assert 类型断言
createTime: 2025/05/02 12:39:25
permalink: /notes/TypeScript/ypo19ld1/
copyright:
  creation: reprint
  license: CC-BY-4.0
...
::: info
类型断言有点类似于 Java 中的==强制类型转换==，但是与 Java 不同的是，类型断言只在编译阶段起作用，辅助我们规范代码，编译完成后会被删除。
:::
...
类型断言有两种语法，一种是“尖括号”语法，另一种是“as”语法。
...
上面两种语法是等价的，`value`表示值，`Type`表示类型。早期只有语法一，后来因为 TypeScript 开始支持 React 的 JSX 语法（尖括号表示 HTML 元素），为了避免两者冲突，就引入了语法二。
...
类型断言的使用前提是，值的实际类型与断言的类型必须满足一个条件，即类型断言的类型必须是值的父类型，或者值的子类型。
...
也就是说，类型断言要求实际的类型与断言的类型兼容，实际类型可以断言为一个更加宽泛的类型（父类型），也可以断言为一个更加精确的类型（子类型），但不能断言为一个完全无关的类型，这样做是没有实际意义的。
...
注意，`as const`断言只能用于字面量（值），不能用于变量。
