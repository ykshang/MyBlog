---
title: 4. any、unknown、never 类型
createTime: 2025/05/02 12:40:14
permalink: /notes/TypeScript/p4hdktog/
copyright:
  creation: reprint
  license: CC-BY-4.0
...
`never`类型的使用场景，主要是在一些类型运算之中，保证类型运算的完整性，详见后面章节。另外，不可能返回值的函数，返回值的类型就可以写成`never`，详见[《函数》](./8.function%20函数.md)一章。
