---
title: 14. 类型映射
createTime: 2025/06/02 23:21:07
permalink: /notes/TypeScript/4hjw94r3/
---
...
TypeScript 4.1 引入了键名重映射（key remapping），允许改变键名。使用 `as 新类型` 子句。
...
键名重映射还可以过滤掉某些属性。下面的例子是只保留 `string` 属性。
