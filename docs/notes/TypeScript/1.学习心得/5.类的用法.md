...
类的成员还可以包含存取器（`accessor`），他是一种特殊的方法，主要用作读取或写入属性时执行额外的逻辑，比如验证或转换。

- 取值器（`getter`）: 用于读取属性，并可以执行一些处理逻辑，例如：格式转换。
- 存值器（`setter`）: 用于写入属性，并可以执行一些处理逻辑，例如：数据验证。
...
::: note 在使用存取器时，我们要注意一些规则：

- 规则 1：如果某个属性对应的只有`get`，没有`set`，则该属性是只读的。
- 规则 2：`get`和`set`方法的可访问性修饰符必须相同，要么都是公开的，要么都是私有的。
- 规则 3：在`5.1`版本之前，`set`的类型必须兼容`get`的的类型，否则报错。`5.1` 版本之后，这个限制被放宽，允许二者类型不兼容。

:::

::: note 规则 3 主要涉及到一种特殊的情况：

DOM 节点的`style`属性，以下是他的类型定义：
...
以上代码中，我们可以看到`CSSStyleDeclaration`是一个对象类型，而`string`是一个字符串类型，它们之间是不兼容的。因此，在`5.1`版本之后，TypeScript 放宽了这个限制，允许`set`和`get`的类型不兼容，但是需要像前边的`CSSStyleRule`接口一样，在定义属性的时候 **显式指定** 对应类型。
