...
- 子类 `B` 的**方法** `show` 的 **参数类型** 和 **返回值类型** 是 `A` 的子类型。类型定义更加严格
...
1. 接口中所有必选的成员都应该被实现；
2. 可选 `?` 的成员可以实现也可以不实现，也可以在实现类中变成必选；
3. 类可以实现接口的同时添加自己的额外成员；
4. 属性的类型定义更加严格
5. 方法的返回值类型更加严格
6. 方法的参数数量更加严格，可以一致，也可以减少。
7. 方法的参数类型更加宽泛，可以一致，也可以增加。
...
interface Person {
  ...
  speak: (val: Parent) => string;
  stand: (val: number) => Child;
}
class Man implements Person {
  ...
  gender: string;
  skills: Parent;
  walk: (val: number) => string;
  run: (val: number | string) => string;
  speak: (val: Child) => string;
  sleep: (val: number) => string;
