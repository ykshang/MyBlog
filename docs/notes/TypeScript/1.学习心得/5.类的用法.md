...
- 无参构造函数：我们可以省略不写，TypeScript 会自动帮我们生成一个默认的无参构造函数。
- 有参构造函数：不能省略不写，因为实例化要传入参数。
...
在 TypeScript 中，`super` 关键字用于访问和调用父类的属性和方法，在类继承中扮演着重要角色：
...
- 子类调用父类构造函数的过程，就是子类继承父类属性和方法的过程。
- 如果类中没有显式的定义构造函数，TypeScrip 会自动生成一个默认的构造函数。
- 当子类和父类都没有显式的定义构造函数，TypeScript 不仅会自动补充，还会自动在子类的构造函数中执行`super()`，调用父类的构造函数。
- 如果子类显式的定义了构造函数，无论父类是否有显式的定义构造函数，子类的构造函数中都必须手动调用父类的构造函数（`super()`），否则会报错。

### 3、同名属性

#### 1、类型兼容

同名属性的类型兼容性要满足：**子类中的属性类型是父类中的属性类型的子类型**。
...
同名属性的访问性要满足：**子类中的属性访问性不能比父类中的属性访问性更严格**。
...
根据结构类型原则，子类的结构一定与父类的结构兼容。因此，子类可以用来给父类赋值。
