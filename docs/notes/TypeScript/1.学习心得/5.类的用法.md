...
当类缺少构造函数定义，TypeScript 会自动添加一个构造函数。在继承的过程中，TypeScript 还能自动在子类中添加`super()`，这个`super()`可能带参数，也可能不带参数，取决于父类的构造函数是否有参数。
...
#### 2、`super.method` 访问父类的方法

`super` 用于调用父类的方法，但不能直接访问父类的属性（除非通过 父类的公开方法间接访问）。
...
子类只能在静态方法中访问父类的静态方法，不能在实例方法中访问父类的静态方法。
