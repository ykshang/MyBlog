...
于是 TypeScript 提供了一种新的简写形式：通过在构造函数参数前添加访问修饰符（`public` / `private` / `protected` / `readonly`），TypeScript 会自动将参数转换为同名实例属性。
...
`readonly`修饰符用于指定属性只能在构造函数中初始化，不能在类的其他地方修改。
...
`readonly`修饰符可以与其他可访问性修饰符一起使用，例如：

```ts
class Point {
  public readonly x: number = 1;
  protected readonly y: number = 1;
  private readonly z: number = 1;
}
```
