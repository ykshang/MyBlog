...
接口可以使用`extends`关键字，继承其他类型，从继承的类型里面拷贝属性类型，这样就不必书写重复的属性。

### 1、可以继承的类型

`interface` 可以继承其他类型，主要有以下三种：
...
`interface` 还可以继承 `class`，即继承该类的所有`public`成员，无法继承对方的私有成员。
...
某些类拥有私有成员和保护成员，interface 可以继承这样的类，但是因为无法实现而报错，意义不大。
...
如果接口和继承目标之间存在同名属性，则==类型必须兼容==（存在继承关系）。即子接口的同名属性类型必须是继承目标的同名属性类型的子类型。
...
接口在多重继承时，==继承目标之间== 如果有同名属性，则 ==类型必须相同==。
