...
函数的类型声明，需要在声明函数时，给出参数的类型和返回值的类型。一般这几种方式：
...
其中写法二中的 **`(txt: string) => void`** 是一个完整的函数类型定义。因此也可以这样写：
...
在实际的函数定义里，参数名与函数类型定义的参数名并不一定相同，只要参数的对应类型相同即可。

```ts
type fun_type = (x: number, y: string) => number;
let myFunc: fun_type = function (a: number) {
  return 11223344;
};
```
...
下面示例中，变量 `myFunc` 的类型只能接受两个参数，如果被赋值为只有一个参数的函数，并不报错。但是，被赋值为有三个参数的函数，就会报错。

```ts twoslash
// @errors: 2322
type fun_type = (x: number, y: number) => number;
...
JavaScript 中允许函数的入参使用别名，即给参数指定一个别名。但是这个与 TypeScript 中的类型定义有冲突。

比如，下面的代码是 JavaScript 允许的。
...
// @errors:  2693
// @noErrors: 7006
// @log: 增加 valA 的定义只是为了让 TypeScript 报错，赋值该场景的理解
type valA = number;
function sum(a: valA, b) {
  console.log(valA + b);
}
...
在 TypeScript 中，`a: number` 被类型定义占用了，`(a: number): valA` 被函数定义占用了，因此我们使用参数解构和函数别名的时候，会出现冲突。

为了解决以上问题，我们可以使用如下的写法。

```ts
function sum({ a: valA, b }: { a: number; b: number }) {
  console.log(valA + b);
}
sum({ a: 1, b: 2 });
```
