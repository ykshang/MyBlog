...
Class 选择器并不是唯一可以被延伸 (extend) 的，Sass 允许延伸任何定义给单个元素的选择器，比如 `.special.cool`，`a:hover` 或者 `a.user[href^="https://"]` 等，例如：
...
SassScript 提供了一些基础的控制指令，比如在满足一定条件时引用样式，或者设定范围重复输出格式。控制指令是一种高级功能，日常编写过程中并不常用到，主要与混合指令 (mixin) 配合使用，尤其是用在 [Compass](https://compass-style.org/) 等样式库中。
...
通过 Ruby API 可以自定义 Sass 函数，具体请查看 [source documentation](https://sass-lang.com/docs/yardoc/Sass/Script/Functions.html#adding_custom_functions)。
