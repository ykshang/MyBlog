...
body {
  background-color: $primary; // 使用配置的变量
}
```
...
| 指令            | 用途           | 示例                                       |
| --------------- | -------------- | ------------------------------------------ |
| **`@function`** | 定义自定义函数 | `@function double($n) { @return $n * 2; }` |
...
| 指令                | 用途           | 示例                                   |
| ------------------- | -------------- | -------------------------------------- |
| **`@if` / `@else`** | 条件分支       | `@if $size > 10 { ... } @else { ... }` |
| **`@each`**         | 遍历列表或 Map | `@each $color in $list { ... }`        |
| **`@for`**          | 数值范围循环   | `@for $i from 1 to 3 { ... }`          |
| **`@while`**        | 条件循环       | `@while $i > 0 { ... }`                |
...
| 指令             | 用途                      | 示例                                |
| ---------------- | ------------------------- | ----------------------------------- |
| **`@at-root`**   | 将嵌套代码提升到根层级    | `@at-root .child { ... }`           |
| **`@media`**     | 嵌套媒体查询（Sass 增强） | `@media (width > 600px) { ... }`    |
| **`@supports`**  | 特性检测嵌套              | `@supports (display: grid) { ... }` |
| **`@keyframes`** | 动画关键帧（支持嵌套）    | `@keyframes slide { ... }`          |
...
| 指令                            | 用途                 | 示例                                |
| ------------------------------- | -------------------- | ----------------------------------- |
| **`@extend !optional`**         | 避免继承失败报错     | `@extend .missing !optional;`       |
| **`@at-root (without: media)`** | 指定跳出某些嵌套层级 | `@at-root (without: media) { ... }` |
